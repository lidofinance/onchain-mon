...
func NewNotificationChannels(log *slog.Logger, cfg *NotificationConfig, httpClient *http.Client, metricsStore *metrics.Store, source string, chainScanner string) (*NotificationChannels, error) {
	...
	for _, tgChannel := range cfg.TelegramChannels {
		channels.TelegramChannels[tgChannel.ID] = notifiler.NewTelegram(tgChannel.BotToken, tgChannel.ChatID, httpClient, metricsStore, source, chainScanner)
	...
	for _, discordChannel := range cfg.DiscordChannels {
		channels.DiscordChannels[discordChannel.ID] = notifiler.NewDiscord(discordChannel.WebhookURL, httpClient, metricsStore, source, chainScanner)
	...
	for _, opsGenieChannel := range cfg.OpsGenieChannels {
		channels.OpsGenieChannels[opsGenieChannel.ID] = notifiler.NewOpsgenie(opsGenieChannel.APIKey, httpClient, metricsStore, source, chainScanner)
